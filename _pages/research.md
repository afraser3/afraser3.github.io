...
## Double-diffusive convection and chemical mixing in stars (or oceans, or gas giant planets)
...
### My latest result: bridging the Prandtl number gap with 3D simulations of astrophysical regimes
*This work was recently highlighted by AAS Nova with a (very well-written) press release: [link](https://aasnova.org/2026/05/06/simulations-of-turbulent-mixing-close-one-gap-but-open-another/)*

The dynamics of DDC are very sensitive to the kinematic viscosity and thermal diffusivity of the fluid, and it can be helpful to characterize various fluids by the ratio of viscosity to diffusivity, which we call the Prandtl number, Pr. In oceans, Pr is roughly 7, while Pr is as small as 10^-6 in stars, so one might expect the fluid motions to be dramatically different. Unfortunately, 3D simulations to date have been restricted to Pr ~ 10^-2 or larger, so we haven't been able to directly probe stellar regimes with 3D simulations. Instead, people typically run simulations at larger Pr, develop reduced models (parameterizations) based off those simulations, and extrapolate the models to 10^-6 to speculate how DDC behaves in stars. This, understandably, is a little dissatisfying and even uncomfortable for stellar astrophysicists who incorporate these sorts of reduced models into their stellar evolution calculations. 

In [this paper](https://ui.adsabs.harvard.edu/abs/2026arXiv260302306F/abstract) (accepted for publication in ApJL), I show that 3D simulations at Pr ~ 10^-6 are, in fact, entirely doable. I present simulations from Pr = 10^-1 down to 10^-6, and show that the reduced model of [Brown, Garaud, & Stellmach (2013)](https://ui.adsabs.harvard.edu/abs/2013ApJ...768...34B/abstract) performs just as well at large Pr as it does at small Pr. This means that, when the results of 3D DDC simulations introduce tension with, e.g., observations, that tension cannot be attributed to a Pr gap between simulations and actual stars, since the models *do* extrapolate very well down to 10^-6. Instead, 1D stellar modelers should look to magnetic fields and maybe rotation or convectively excited waves to resolve these tensions.

### My other latest result: maximally helical (Beltrami) flows spontaneously formed by salt fingers
